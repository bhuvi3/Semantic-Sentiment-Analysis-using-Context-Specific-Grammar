...
conf = (SparkConf().setMaster("local[8]").setAppName("lexical_sentiment_analysis_linear_regression_SENTI_SCALE").set("spark.executor.memory", "1g"))
sc = SparkContext(conf = conf)

training_data = sc.textFile('../imdb/supervised/featurized_train_data_SENTI_SCALE.csv')
testing_data = sc.textFile('../imdb/supervised/featurized_test_data_SENTI_SCALE.csv')
...
def mapper_CF(x):
    ...
    c = len(tokens)
    total_attrib = 14
    class_index = 13 #rating
    feature_index_start, feature_index_end = (1, 11) #included
    if c != total_attrib:
...
def mapper_IP(x):
    ...
    total_attrib = 14
    class_index = 13 #rating
    feature_index_start, feature_index_end = (1, 11)
    if c != total_attrib:
        print "*********DATA VALIDATION ERROR\n************"
    attrib = [float(y) for y in tokens]
    #attrib[11] = int(attrib[11])
    ref_id = attrib[0]
    actual_polarity = attrib[12]
    return LabeledPoint(ref_id, actual_polarity)

vectorize_start = time.time()
vectorized_data = training_data.map(mapper_CF)
vectorized_testing_data = testing_data.map(mapper_CF)

polarity_train_actual = training_data.map(mapper_IP)
polarity_test_actual = testing_data.map(mapper_IP)
"""
...
model = LinearRegressionWithSGD.train(vectorized_data)
train_end = time.time()
print "******************MODEL TRAINING: DONE********************"

#predicting rating for testing data and evaluating
...
#predicting polarity for testing data and evaluating
def mapper_predict_polarity(x):
    z = str(x)
    tokens = z.split(',')
    c = len(tokens)
    total_attrib = 14
    ...
    if c != total_attrib:
        print "*********DATA VALIDATION ERROR\n************"
    attrib = [float(y) for y in tokens]
    features = attrib[feature_index_start:(feature_index_end+1)]
    #attrib[11] = int(attrib[11])
    ref_id = attrib[0]
    actual_polarity = attrib[12]
    #predicted_class = int(round(predicted_class))
    predicted_class = model.predict(features)
    predicted_rating = float(str(predicted_class))
    if predicted_rating <= 5:
        predicted_polarity = 'neg'
    else:
        predicted_polarity = 'pos'
    return (actual_polarity, predicted_polarity)

pred_start = time.time()
actual_and_predicted = vectorized_testing_data.map(mapper_predict)
#polarity_test_predicted = vectorized_testing_data.map(mapper_predict_polarity)
polarity_actual_and_predicted = testing_data.map(mapper_predict_polarity)
...
training_error = polarity_actual_and_predicted.filter(lambda (a, p): a != p).count() / float(count)
...
print "******************RESULTS********************\n"
heading = "***LEXICAL SENTIMENT SCALE ANALYSIS***\n"
#"""
...
data_info = data_title + instances
#"""
title = "***LINEAR REGRESSION SENTI_SCALE RESULTS***\n"
...
res_fh = open('../result/linear_regression_result_SENTI_SCALE.txt','w')
res_fh.write(result)
res_fh.close()
